...
func (h *AdminHandler) DeleteGrant(w http.ResponseWriter, r *http.Request) {
	...
	id, err := uuid.Parse(idStr)
	if err != nil {
		h.Log().Error("invalid grant ID", "error", err, "id", idStr)
...
func (h *AdminHandler) Log() core.Logger {
	return h.xparams.Log()
}

func (h *AdminHandler) Cfg() *config.Config {
	return h.xparams.Cfg()
}

func (h *AdminHandler) Trace() core.Tracer {
